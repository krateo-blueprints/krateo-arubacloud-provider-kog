...
def main():
    ...
    for prov, fn in PROVIDERS.items():
        ...
        # OKF frontmatter: the shared lint-docs workflow requires type/title/description
        # on every docs/**/*.md, generated files included.
        out = ["---",
               "type: Architecture",
        ...
        out.append(f"- **OpenAPI**: `{info.get('title')}` v{info.get('version')} "
                   ...
                   f"- **Security scheme**: `{', '.join(ss)}` (HTTP Bearer)\n"
